...
TestDiff = Test.Unit.TestCase.extend({
  setup: function () {
    set_file_paths()
    @git = Git.open(@wdir)
    @diff = @git.diff("gitsearch1", "v2.5")
  },
  test_diff_tags: function () {
  ...
  test_diff_objects: function () {
    d = @git.diff("gitsearch1", @git.gtree("v2.5"))
    assert(3, d.size())
  },
  test_object_diff: function () {
  ...
  test_diff_each: function () {
    files = {  }
    @diff.each { |d| files[d.path()] = d }
