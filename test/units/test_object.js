...
TestObject = Test.Unit.TestCase.extend({
  setup: function () {
    set_file_paths()
    @git = Git.open(@wdir)
  ...
  test_grep: function () {
    g = @git.gtree("a3db7143944dcfa0").grep("search")
    assert_equal(3, g.to_a().flatten().size())
    assert_equal(1, g.size())
    assert_equal({}, @git.gtree("a3db7143944dcfa0").grep("34a566d193"))
    g = @git.gcommit("gitsearch1").grep("search")
    assert_equal(8, g.to_a().flatten().size())
    assert_equal(2, g.size())
    g = @git.gcommit("gitsearch1").grep("search", "scott/new*")
    assert_equal(3, g.to_a().flatten().size())
    assert_equal(1, g.size())
  },
});
