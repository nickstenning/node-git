...
class Ruby2Javascript < SexpProcessor
  LINE_LENGTH = 78
  ...
  def process_call(exp)
    ...
    receiver = "(#{receiver})" if
      Ruby2Javascript::ASSIGN_NODES.include? receiver_node_type
  ...
  def process_defined(exp)
    "typeof #{process(exp.shift)} !== 'undefined'"
  end
  ...
  def process_for(exp)
    ...
    result = ["for (#{iter} in #{recv}) { // FIXME: Check this loop!"]
  ...
  def process_if(exp)
    expand = Ruby2Javascript::ASSIGN_NODES.include? exp.first.first
    c = process exp.shift
    t = process exp.shift
    f = process exp.shift
  ...
  def util_class(exp)
    ...
    namespace(name) do |name|
      result << "#{name} = #{superk or 'Class'}.extend({"
  ...
  def namespace(name, &block)
    if name and block_given?
      @namespace ||= []
      @namespace.push(name)
      res = block.call(@namespace.join('.'))
      @namespace.pop
      return res
    else
